...
int main (int argc, char * argv[]) {
    ...
    timer.start();
    // Création d'un stream non par défaut
    cudaStream_t stream;
    cudaStreamCreate(&stream);
    // Transfert Hôte -> Device dans le stream non par défaut
    cudaMemcpyAsync(data_gpu, data_cpu, 
               sizeof(uint64_t)*num_entries, cudaMemcpyHostToDevice, stream);
    check_last_error();
    // Lancement du kernel dans le stream non par défaut (après le memcpy)
    decrypt_gpu<<<80*32, 64, 0, stream>>>(data_gpu, num_entries, num_iters);
    check_last_error();
    // Transfert Device -> Hôte dans le stream non par défaut
    cudaMemcpyAsync(data_cpu, data_gpu, 
               sizeof(uint64_t)*num_entries, cudaMemcpyDeviceToHost, stream);
    // Synchronisation du stream avant de continuer
    ...
    cudaFree    (data_gpu);
    // Destruction du stream non par défaut
