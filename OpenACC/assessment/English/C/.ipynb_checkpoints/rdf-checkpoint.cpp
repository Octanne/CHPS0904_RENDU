...
int main(int argc, char *argv[])
{
	...
	//////////////////////////////////////////////////////////////////////////
    #pragma acc data copyin(h_x[0:nconf*numatm], h_y[0:nconf*numatm], h_z[0:nconf*numatm]) copy(h_g2[0:nbin])
    {
...
{
    double box = fmin(fmin(xbox, ybox), zbox);
    double del = box / (2.0 * d_bin);
    double cut = box * 0.5;

    #pragma acc parallel loop collapse(2) present(d_x[0:nconf*numatm], d_y[0:nconf*numatm], d_z[0:nconf*numatm], d_g2[0:d_bin])
              ...
              const double xbox, const double ybox, const double zbox, int d_bin)
{
    ...
    #pragma acc parallel loop gang collapse(2) present(d_x[0:nconf*numatm], d_y[0:nconf*numatm], d_z[0:nconf*numatm], d_g2[0:d_bin])
    for (int frame = 0; frame < nconf; frame++)
    {
        for (int id1 = 0; id1 < numatm; id1++)
        {
