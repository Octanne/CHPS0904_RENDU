...
// Macro pour vérifier les appels MPI et afficher une erreur en cas d'échec
...
// Macro pour vérifier les appels CUDA Runtime
...
// Macro pour vérifier les appels NCCL
...
// Définition des versions NCCL pour la gestion des buffers utilisateurs
#define NCCL_VERSION_UB NCCL_VERSION(2,19,1)
#define NCCL_UB_SUPPORT NCCL_VERSION_CODE >= NCCL_VERSION_UB

// Choix du type réel (float ou double)
#ifdef USE_DOUBLE
typedef double real;
...
constexpr real tol = 1.0e-8; // Tolérance pour la convergence

const real PI = 2.0 * std::asin(1.0); // Valeur de PI

// Prototypes des fonctions CUDA pour initialiser les frontières et effectuer le calcul Jacobi
...
// Fonction pour exécuter la version mono-GPU et obtenir la référence séquentielle
double single_gpu(const int nx, const int ny, const int iter_max, real* const a_ref_h,
          const int nccheck, const bool print);

// Fonction utilitaire pour lire un argument de la ligne de commande
template <typename T>
...
// Fonction utilitaire pour vérifier la présence d'un argument
...
int main(int argc, char* argv[]) {
  ...
  // Initialisation de NCCL (récupération et diffusion de l'identifiant unique)
  ...
  // Détermination du rang local et du nombre de processus locaux sur le noeud
  int local_rank = -1;
  int local_size = 1;
  {
    MPI_Comm local_comm;
  ...
  // Vérification du nombre de GPUs visibles
  ...
  // Attribution du GPU à chaque processus
  if ( 1 == num_devices ) {
  ...
  // Initialisation du communicateur NCCL
  ...
  // Allocation mémoire pour la référence séquentielle et la solution parallèle
  real* a_ref_h;
  ...
  // Découpage du domaine en bandes pour chaque processus
  int chunk_size;
  int chunk_size_low = (ny - 2) / size;
  int chunk_size_high = chunk_size_low + 1;
  int num_ranks_low = size * chunk_size_low + size -
            (ny - 2);  // Nombre de rangs avec chunk_size = chunk_size_low
  if (rank < num_ranks_low)
    chunk_size = chunk_size_low;
  else
    chunk_size = chunk_size_high;

  // Allocation mémoire pour les tableaux locaux (avec ou sans buffer registration)
  real* a;
  real* a_new;
#if NCCL_UB_SUPPORT
  void* a_reg_handle;
  ...
  // Calcul des indices de début et de fin dans le tableau global
  int iy_start_global;  // Mon indice de début dans le tableau global
  if (rank < num_ranks_low) {
  ...
  int iy_end_global = iy_start_global + chunk_size - 1;  // Mon dernier indice dans le tableau global

  int iy_start = 1;
  int iy_end = iy_start + chunk_size;

  // Initialisation des conditions de Dirichlet sur les bords gauche et droit
  launch_initialize_boundaries(a, a_new, PI, iy_start_global - 1, nx, (chunk_size + 2), ny);
  CUDA_RT_CALL(cudaDeviceSynchronize());

  // Création des streams et événements CUDA
  ...
  // Allocation mémoire pour le calcul de la norme L2
  real* l2_norm_d;
  CUDA_RT_CALL(cudaMalloc(&l2_norm_d, sizeof(real)));
  real* l2_norm_h;
  CUDA_RT_CALL(cudaMallocHost(&l2_norm_h, sizeof(real)));

  // Phase de "warmup" NCCL pour stabiliser les performances
  ...
  int iter = 0;
  real l2_norm = 1.0;
  bool calculate_norm = true; // Indique si la norme L2 doit être calculée à cette itération

  MPI_CALL(MPI_Barrier(MPI_COMM_WORLD));
  double start = MPI_Wtime();
  PUSH_RANGE("Jacobi solve", 0)
  // Boucle principale de l'algorithme de Jacobi parallèle
  while (l2_norm > tol && iter < iter_max) {
    CUDA_RT_CALL(cudaMemsetAsync(l2_norm_d, 0, sizeof(real), compute_stream));

    calculate_norm = (iter % nccheck) == 0 || (!csv && (iter % 100) == 0);

    // Lancement du kernel Jacobi
    ...
    // Communication des bandes frontières avec les voisins (conditions périodiques)
  ...
  // Récupération des résultats sur l'hôte pour vérification
  CUDA_RT_CALL(cudaMemcpy(a_h + iy_start_global * nx, a + nx,
              std::min((ny - iy_start_global) * nx, chunk_size * nx) * sizeof(real),
              cudaMemcpyDeviceToHost));

  // Vérification de la solution parallèle par rapport à la référence séquentielle
  ...
  // Réduction MPI pour vérifier si tous les processus ont trouvé la bonne solution
  ...
  // Libération des ressources CUDA et NCCL
...
// Version séquentielle sur un seul GPU pour obtenir la référence
double single_gpu(const int nx, const int ny, const int iter_max, real* const a_ref_h,
          const int nccheck, const bool print) {
  real* a;
  real* a_new;
  ...
  // Initialisation des conditions de Dirichlet sur les bords
  ...
  double start = MPI_Wtime();
  PUSH_RANGE("Jacobi solve", 0)
  // Boucle principale Jacobi séquentielle
  while (l2_norm > tol && iter < iter_max) {
    CUDA_RT_CALL(cudaMemsetAsync(l2_norm_d, 0, sizeof(real), compute_stream));
    ...
    // Application des conditions périodiques (copie des bandes)
