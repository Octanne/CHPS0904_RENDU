...
// Convertit la chaîne NVSHMEM_SYMMETRIC_SIZE en entier non signé long long
...
// Définition du type réel et de la tolérance
typedef float real;
...
// Initialise les conditions aux limites de Dirichlet sur les bords gauche et droit
...
// Kernel Jacobi principal avec communication point à point via NVSHMEM
...
// Variante du kernel Jacobi utilisant la communication par bloc
template <int BLOCK_DIM_X, int BLOCK_DIM_Y>
...
// Fonction pour exécuter la version mono-GPU de Jacobi et obtenir la référence séquentielle
double single_gpu(const int nx, const int ny, const int iter_max, real* const a_ref_h,
                  const int nccheck, const bool print, int mype);

// Récupère la valeur d'un argument passé en ligne de commande
template <typename T>
...
// Vérifie la présence d'un argument dans la ligne de commande
...
// Structure pour gérer les buffers du calcul du L2-norm
struct l2_norm_buf {
...
// Version séquentielle mono-GPU pour référence
double single_gpu(const int nx, const int ny, const int iter_max, real* const a_ref_h,
                  const int nccheck, const bool print, int mype) {
    real* a;
    real* a_new;
