...
int main(int argc, char* argv[]) {
    ...
    // Synchronisation de tous les processus après la diffusion du nccl_uid
    ...
    // Détermination du rang local et du nombre de processus locaux
    int local_rank = -1;
    int local_size = 1;
    {
        MPI_Comm local_comm;
    ...
    // Vérification du nombre de GPU visibles
    ...
    if ( 1 == num_devices ) {
        // Un seul GPU visible, on suppose que l'affinité GPU est gérée via CUDA_VISIBLE_DEVICES
    ...
    if ( nccl_version < 21501 ) {
        fprintf(stderr,"ERROR NCCL 2.15.1 or newer is required.\n");
        NCCL_CALL(ncclCommDestroy(nccl_comm));
        MPI_CALL(MPI_Finalize());
        return 1;
    }
    int driverVersion = 0;
    CUDA_RT_CALL(cudaDriverGetVersion (&driverVersion ));
    if ( driverVersion < 11070 ) {
        fprintf(stderr,"ERROR Driver with support for CUDA >= 11.7 is required (%d.%d supported by install)\n", driverVersion/1000, (driverVersion%1000)/10 );
        NCCL_CALL(ncclCommDestroy(nccl_comm));
        MPI_CALL(MPI_Finalize());
        return 1;
    }

    // Allocation mémoire pour la référence et la solution
    real* a_ref_h;
    ...
    // Allocation mémoire pour les tableaux locaux
    real* a;
    real* a_new;
#if NCCL_UB_SUPPORT
    void* a_reg_handle;
    ...
    if (user_buffer_reg) {
        ...
        // ncclCommRegister n'est pas requis si les kernels NCCL sont lancés avec un CUDA graph depuis un stream capture
    ...
    PUSH_RANGE("Build graphs", 0)
    {
        ...
        // cudaGraph_t objects are no longer required after instantiation
    ...
    CUDA_RT_CALL(cudaStreamSynchronize(compute_stream));
    CUDA_RT_CALL(cudaGraphLaunch(graph_exec[0], compute_stream));
    CUDA_RT_CALL(cudaGraphLaunch(graph_exec[1], compute_stream));
    CUDA_RT_CALL(cudaGraphLaunch(graph_calc_norm_exec[0], compute_stream));
    CUDA_RT_CALL(cudaGraphLaunch(graph_calc_norm_exec[1], compute_stream));
    CUDA_RT_CALL(cudaStreamSynchronize(compute_stream));
    POP_RANGE
    ...
    if (rank == 0 && result_correct) {
        if (csv) {
            printf("nccl_graphs, %d, %d, %d, %d, %d, 1, %f, %f\n", nx, ny, iter_max, nccheck, size,
...
// Fonction pour exécuter la relaxation de Jacobi sur un seul GPU
double single_gpu(const int nx, const int ny, const int iter_max, real* const a_ref_h,
                  const int nccheck, const bool print) {
    real* a;
    real* a_new;
    ...
    // Boucle principale de relaxation de Jacobi
    double start = MPI_Wtime();
    PUSH_RANGE("Jacobi solve", 0)
    while (l2_norm > tol && iter < iter_max) {
        CUDA_RT_CALL(cudaMemsetAsync(l2_norm_d, 0, sizeof(real), compute_stream));
        ...
        // Application des conditions périodiques aux frontières
        ...
        if (calculate_norm) {
            ...
            l2_norm = *l2_norm_h;
            l2_norm = std::sqrt(l2_norm);
            // Affichage de la norme L2 toutes les 100 itérations
    ...
    // Copie du résultat final sur l'hôte
